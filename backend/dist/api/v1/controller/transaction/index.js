...
class TransactionController {
    ...
    async getTransitionsByUserId(req, res, next) {
        try {
            ...
            return (0, expressUtils_1.Respond)({
                ...
                status: 200,
                data: {
                    transactions: transactions.map(({ user, book, id, dueDate, transactionType, issueDate }) => ({
                        user,
                        book,
                        id,
                        dueDate,
    ...
    async creteTransition(req, res, next) {
        try {
            ...
            const { user, book, id, dueDate, transactionType, issueDate } = await transaction_1.default.createTransaction(req.body);
            return (0, expressUtils_1.Respond)({
                res,
                status: 201,
    ...
    async getTransitionByID(req, res, next) {
        try {
            ...
            const { user, book, id, dueDate, transactionType, issueDate } = await transaction_1.default.getTransactionById(new mongoose_1.Types.ObjectId(Id));
            return (0, expressUtils_1.Respond)({
                res,
                status: 201,
                data: { user, book, id, dueDate, transactionType, issueDate },
    ...
    async getAllTransitions(req, res, next) {
        try {
            ...
            return (0, expressUtils_1.Respond)({
                res,
                status: 200,
                data: { transactions: transactions.map(({ user, book, id, dueDate, transactionType, issueDate }) => ({
                        user,
                        book,
                        id,
                        dueDate,
